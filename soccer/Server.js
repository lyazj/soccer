...
http.createServer(function (request, response) {
  ...
  fs.stat(pathname, function (err, stats) {
    ...
    if(stats.isDirectory())
    {
      ...
      if(query.mode == "list")
        return fs.readdir(pathname, function (err, files) {
          ...
          response.writeHead(200, {"Content-Type": fileTypes.json})
          var result = {
      ...
      response.writeHead(200, {"Content-Type": fileTypes.html})
    ...
    for(var type in fileTypes)
      ...
      {
        response.writeHead(200, {"Content-Type": fileTypes[type]})
